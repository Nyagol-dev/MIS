...
import { listPlans, createPlan } from '@/lib/billing/plans';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    return await withTenantContext(session.tenantId, async (client) => {
      ...
      const result = await listPlans(client, session.tenantId, filters, { limit, offset });
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    return await withTenantContext(session.tenantId, async (client) => {
      ...
      const plan = await createPlan(client, session.tenantId, body);
