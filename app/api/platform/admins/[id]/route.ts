...
import { deactivatePlatformAdmin } from "@/lib/platform/platformAdmins";
...
export async function DELETE(
  ...
  try {
    ...
    const result = await deactivatePlatformAdmin(session, (await props.params).id);
    
    if (result && 'code' in result) {
      if (result.code === 'PLATFORM_ADMIN_NOT_FOUND') {
        return NextResponse.json({ error: result.message }, { status: 404 });
      }
      if (result.code === 'CANNOT_DEACTIVATE_SELF') {
