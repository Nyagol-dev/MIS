...
import { getUser, updateUserProfile, deactivateUser } from '@/lib/users/users';
...
export async function GET(
  ...
  try {
    ...
    return await withTenantContext(session.tenantId, async (client) => {
      ...
      const result = await getUser(client, session.tenantId, id);
...
export async function PATCH(
  ...
  try {
    ...
    return await withTenantContext(session.tenantId, async (client) => {
      ...
      const result = await updateUserProfile(client, session.tenantId, id, body);
      return NextResponse.json(result || {}, { status: 200 });
    });
  } catch (error) {
    return handleError(error);
...
export async function DELETE(
  ...
  try {
    ...
    return await withTenantContext(session.tenantId, async (client) => {
      ...
      await deactivateUser(client, session.tenantId, id, session.userId);
